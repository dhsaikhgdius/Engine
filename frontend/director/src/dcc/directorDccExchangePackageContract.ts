import { z } from "zod";
import { directorProjectSchema } from "../comprehensive/editor/schema/directorProjectSchema";
...
/** Describes a single exchange-format artifact within a package. */
...
/** Describes a single bundled asset within an exchange package. */
...
/**
 * Exchange package manifest schema.
 *
 * Describes a portable export package containing the full Director project
 * plus exchange-format artifacts (glb, usda). Validated at the gateway
 * boundary before being handed to a DCC provider.
 */
...
/**
 * Exchange package result schema.
 *
 * Returned by a DCC provider after consuming an exchange package. Carries
 * the output artifacts, their hashes, and any warnings.
 */
...
/** Inferred type for a validated exchange package manifest. */
export type DirectorDccExchangePackageManifest = z.infer<typeof directorDccExchangePackageManifestSchema>;
/** Inferred type for a validated exchange package result. */
