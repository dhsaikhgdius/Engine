...
- White-box is a metric clay look with readable silhouettes, not a modeling method and not a pile of primitives. Search the catalog first and place matches with author.add_object, keeping imported architecture at its authored metric scale (describe author.add_object covers modelNormalization). Model missing architecture with blender_native create_blockout; cut doors and windows with create_opening or a BOOLEAN modifier, never a darker box on a wall.
