...
- Game work uses an opt-in engine workshop; film delivery keeps the reviewed snapshot/patch path above. `render_engine_frame` is an independent visual acceptance primitive for Unreal, Unity, and Godot. `start_engine_session` adopts an already-open Unity or Godot editor, or Unreal's token-gated loopback listener (`port` required). `allow_code:true` enables provider-native `execute_code` (C#, GDScript, or Editor Python); Unreal additionally requires the listener's `--preview-allow-code` flag. `authority:"engine"` enables `sync_scene`, and `sync_engine_session_to_director` applies only the stable-ID review projection under revision/idempotency guards; scripts, prefabs, collision, navigation, UI, and other native state remain authoritative in the engine. Default sessions are Director-authoritative with code disabled.
- Local engine process operations (the Director-native equivalent of the community godot-mcp / unity-mcp launch-and-run tools): `launch_engine_editor` opens the configured engine project in its editor GUI (fixed argument vector, detached); `run_engine_project` runs the configured project and captures a bounded stdout/stderr tail — Godot today, with optional `scene` (a `res://` path) and `headless`; Unity and Unreal return structured `engine_run_unsupported` diagnostics with recovery steps. Poll `engine_run_status` for the output tail and stop with `stop_engine_project` (SIGTERM → SIGKILL). Runs execute the user's own local project and never accept request-supplied scripts.
- In an engine workshop, queue `capture_frame` (Unity/Godot), `execute_code`, or `sync_scene` with `engine_session_command`, poll `engine_session_command_status`, and close with `stop_engine_session`. Unreal clean frames stay on `render_engine_frame`. Never treat a completed command as visual acceptance without a frame.
- Humans see the same operations in the editor's interchange menu under the "DCC / engine handoff" dock (Blender / Unreal / Unity / Godot tabs): provider readiness, connector health and recovery, engine editor launch plus Godot run controls with the bounded output tail, send receipts with structured omitted channels, dry-run return previews, engine scene import (zip upload → reviewed plan → guarded apply), and preview-only live-link status, including the Unreal active-camera push controls. The interchange trigger in the top bar carries a four-host readiness dot cluster. When walking a user through a handoff, point them there rather than at raw HTTP routes.

## Game-engine scene import (Unreal / Unity / Godot)

- Use the **`director_dcc` tool** to bring an Unreal Engine, Unity, or Godot scene into Director. Check the runtime first with `{"op":"status","provider":"unreal"}`, `"unity"`, or `"godot"`: `nativeReady` means the engine executable and its Director connector were both detected; `exchangeReady` means portable `director-engine-scene-v1` `.zip` packages can be imported without the engine.
- Two ways to obtain a package: `{"op":"extract_engine_scene","provider":"unity","project_dir":"<engine project>","scene":"Assets/Scenes/Main.unity"}` runs the installed engine headlessly (Unreal loads `integrations/unreal/interchange/director_scene_export.py`; Unity gets `integrations/unity/interchange/DirectorSceneExport.cs` copied into `Assets/Editor/DirectorInterchange/`; Godot gets `integrations/godot/interchange/director_scene_export.gd` copied into `res://addons/director_interchange/` and accepts `"scene":"res://scenes/main.tscn"`, defaulting to the project's main scene), or upload a `.zip` the exporter wrote inside the engine to `POST /api/dcc/engine-scene/uploads?provider=unreal|unity|godot`. Both return a validated package with a `packageDir`.
...
- Package transforms are already in Director's right-handed Y-up metre convention — the in-engine exporters own the coordinate conversion and declare the linear map in the manifest (Godot's native basis already matches, so its manifest declares the identity map). Geometry, materials, and skinned meshes ride inside `assets/scene.glb`; cameras and lights become native Director cameras and lights. Unreal needs the glTF Exporter plugin enabled and Unity needs `com.unity.cloud.gltfast` for geometry; Godot exports geometry through the built-in GLTFDocument with no extra plugin. Without them the package still imports cameras, lights, and hierarchy with an explicit gap entry. Round-trip back to the engines is `planned`, not available.
...
- `export_slice` still does not synthesize an engine project: it routes a playable Stage slice to `director_dcc`. When the user explicitly chooses an engine-owned game workflow, continue in an opt-in engine workshop and use provider-native code/scene state there; validate with engine capture and a runnable result, and keep Director as the review projection rather than claiming it owns Blueprints, prefabs, scripts, collision, navigation, or UI.
...
- Attach an Agent to a character with author `bind_character_agent` (`object_id` plus `session_id` and/or `profile_id`) and detach with `unbind_character_agent`; observe echoes `agent_binding` on character summaries. Bind the durable session id that will drive the character — the anonymous HTTP fallback `http-default` is rejected as a binding identity. A session that possesses characters may only mutate those characters; global writes such as `start_scene`, `replace_project`, or `reconstruction` `apply` are rejected until the binding is removed, `player` `enter`/`set_actor`/`teleport`/`walk_to` must name a possessed `actor_id`, `enter`/`set_actor` cannot take over a live Player Mode driving an unpossessed actor, and `pilot record_waypoint` is rejected under possession (transient pilot flight stays available). Every possession rejection carries a typed `possession` block next to the prose error (the possessed ids plus the offending operation and reason), so read it instead of parsing prose.
