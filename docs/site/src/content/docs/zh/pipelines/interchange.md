...
| 引擎交接（Unreal/Unity/Godot） | 发送/回传 | 无头连接器导入场景布局、相机与镜头范围并写入 `director:id`；以 canonical 空间回传变换。Unreal 额外将 Gateway 烘焙的变换/相机动画写入 Sequencer（有理帧率、SMPTE 起始时间码），把带蒙皮 GLB 以绑定姿态导入为骨骼网格，将 Director PBR 参数应用为材质实例，提供仅预览的 live-link 相机通道，并可返回可选的哈希锁定干净帧渲染回执 | 需要用户引擎工程中已安装 Director 官方连接器（`nativeReady`）；对 Unity 与 Godot 不宣称动画、骨骼、材质或 live link；Unreal 的 live link 仅为视口预览、绝不修改工程；Unreal 的 Control Rig 姿态、动作片段与贴图以警告省略处理并附结构化省略记录 |
