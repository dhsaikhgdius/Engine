...
| Blender 往返                   | 导出/回传 | 经过验证的场景/相机/灯光交接、clay 预览、按稳定 ID 回传 mesh、变换、机位光学、`director_id` 灯光与可移植 pose control | 仅接受 DCC job 根下带 hash 的受限 package；不自动导入 Blender 游离对象，直接编辑 armature 骨骼只警告、不 reconcile |
| 引擎交接（Unreal/Unity/Godot） | 发送/回传 | 无头连接器导入场景布局、相机与镜头范围并写入 `director:id`；以 canonical 空间回传变换。Unreal 额外将 Gateway 烘焙的变换/相机动画写入 Sequencer（有理帧率、SMPTE 起始时间码），把带蒙皮 GLB 以绑定姿态导入为骨骼网格，将 Director PBR 参数应用为材质实例，提供仅预览的 live-link 相机通道，并可返回可选的哈希锁定干净帧渲染回执；Unity 额外把 Director 动画烘焙到 Timeline、从蒙皮 GLB 构建 Avatar，并应用 PBR 材质回退与灯光；Godot 4 额外导入基于有理时基的 Gateway 烘焙 `AnimationPlayer` 动画、绑定姿态的蒙皮 GLB 骨架、带哈希外置纹理的 `StandardMaterial3D` 材质，以及 Omni/Spot/Directional 灯光 | 需要用户引擎工程中已安装 Director 官方连接器（`nativeReady`）；Unreal 的 live link 仅为视口预览、绝不修改工程，Unity 与 Godot 不宣称 live link；Unreal 的 Control Rig 姿态、动作片段与贴图以警告省略处理并附结构化省略记录；Godot 的绑定姿态通道与环境光/面光警告省略 |
