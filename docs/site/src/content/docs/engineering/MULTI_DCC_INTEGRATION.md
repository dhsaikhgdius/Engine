...
- **Unreal Engine, Unity, and Godot 4** have implemented Director-authored
  **engine-headless connectors**: the Gateway can run a fixed connector entry
  ...
  round trip. Each connector additionally ships a validated host-side subset:
  **Unreal** keys Gateway-baked transform/camera animation into Sequencer,
  imports skinned-GLB skeletal meshes, and instances PBR materials; **Unity**
  bakes animation onto Timeline, builds Humanoid/Generic Avatars from skinned
  GLB, and translates PBR materials; **Godot 4** bakes Gateway animation into
  `AnimationPlayer` tracks on a rational timebase, imports skinned GLB
  skeletons in bind pose, translates `StandardMaterial3D` with hashed external
  textures, and imports Omni/Spot/Directional lights. Live link remains
  **planned** for all three engines; the exchange package can carry model
...
portable format still carries them. Each engine connector additionally
promotes `animation`, `skeleton`, and `materials` to `native` for the subset
its host-free golden fixtures verify: Unreal (Gateway-baked Sequencer tracks,
skinned-GLB skeletal mesh import, PBR material instances;
`backend/gateway/tests/dcc/unreal*.test.ts`), Unity (Timeline animation bakes,
Humanoid/Generic Avatars, PBR material translation;
`backend/gateway/tests/dcc/unity*.test.ts`), and Godot (AnimationPlayer bakes,
skinned GLB `Skeleton3D` import, `StandardMaterial3D` translation;
`backend/gateway/tests/dcc/godot*.test.ts`). `live_link` stays `planned` for
every engine because no Gateway transport ships yet.
...
| Godot 4          | **Implemented headless connector (scene/cameras/animation/skeleton/materials/lights)** | GLB                     | `godot --headless`, GDScript editor plug-ins       | Baked animation, skeletons, materials, lights implemented; live preview still planned | P2       |
...
lossless USD animation, Control Rig transfer, or texture translation. The Unity
connector additionally bakes Director animation onto Timeline, builds
Humanoid/Generic Avatars from skinned GLB, and translates PBR materials, pinned
by the in-package EditMode suite plus the host-free Unity golden tests. The
Godot connector additionally ships Gateway-baked `AnimationPlayer` animation,
skinned GLB `Skeleton3D` import, `StandardMaterial3D` translation with hashed
external textures, and Omni/Spot/Directional lights, backed by host-free
goldens plus a skip-if-missing real headless roundtrip. Each remains a warned,
bounded subset, and live link remains `planned` for every engine.
