...
it("resolves legacy preset-only rigs and keeps explicit controls as sparse overrides", () => {
  ...
  ).toEqual({ ...wave.controls, "head.yaw": 24 });
});

it("keeps every built-in preset inside the same per-control limits exposed to UI and Agents", () => {
  MANNEQUIN_POSE_PRESETS.forEach((preset) => {
    Object.entries(preset.controls).forEach(([control, value]) => {
      const limits = getCharacterPoseControlValueLimits(control);
      expect(value, `${preset.id}.${control}`).toBeGreaterThanOrEqual(limits.min);
      expect(value, `${preset.id}.${control}`).toBeLessThanOrEqual(limits.max);
    });
  });
});

it("includes agent-ready punch and block poses", () => {
  const punch = MANNEQUIN_POSE_PRESETS.find((item) => item.id === "punch");
  const block = MANNEQUIN_POSE_PRESETS.find((item) => item.id === "block");

  expect(punch?.controls["rightShoulder.pitch"]).toBeGreaterThan(80);
  expect(punch?.controls["rightElbow.bend"]).toBeLessThan(20);
  expect(block?.controls["leftElbow.bend"]).toBeGreaterThan(100);
  expect(block?.controls["rightElbow.bend"]).toBeGreaterThan(90);
});
