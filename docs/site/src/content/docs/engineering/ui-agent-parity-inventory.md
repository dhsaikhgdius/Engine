...
- Canvas node/edge/layout/section authoring (`canvas.node.*`, `canvas.section.*`,
  `canvas.node.assign_section`, `canvas.edge.*`, `canvas.dag.layout`), media import
  cataloging (`gallery.media.update`), and the undo/redo buttons (`workspace.undo` /
  `workspace.redo`). Observe exposes `board.sections` and `node.section_id`.
...
- Continuous interactions — node drags, clip drags/trims, fade drags, range sliders, live
  ...
  shared above).
- Canvas z-order raises and view state — no semantic operations yet (section add/assign/update/remove are shared).
