...
function getFrustumMetrics({ fov = 50, target = [0, 1.2, 0] }: { fov?: number; target?: [number, number, number] }) {
...
it("uses the current long rectangular camera body behind the shot origin", () => {
  ...
  const bodyHeight =
    Math.max(...bodyPoints.map((point) => point[1])) - Math.min(...bodyPoints.map((point) => point[1]));
...
it("places the two camera reels as separated vertical disks along the long body", () => {
  ...
  const reelXRange =
    Math.max(...reelLines.flatMap((line) => line.points.map((point) => point[0]))) -
...
it("draws the viewport camera lens as a deeper faceted front assembly facing the viewfinder", () => {
  ...
  const lensFrontCenter = frontFramePoints
    .reduce((acc, point) => [acc[0] + point[0], acc[1] + point[1], acc[2] + point[2]] as [number, number, number], [
      0, 0, 0,
...
it("starts the viewfinder lines from the camera transform center", () => {
