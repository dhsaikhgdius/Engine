...
| 引擎交接（Unreal/Unity/Godot） | 发送/回传 | 无头连接器导入场景布局、相机与镜头范围并写入 `director:id`；以 canonical 空间回传变换；Unity 额外把 Director 动画与语义姿势通道烘焙到 Timeline、从蒙皮 GLB 构建 Avatar、应用 PBR 材质回退与灯光，并提供仅出站的预览 live link | 需要用户引擎工程中已安装 Director 官方连接器（`nativeReady`）；Unreal 与 Godot 不宣称动画、骨骼、材质或 live link；Unity 的 live link 仅用于预览，绝不作为场景权威 |
