...
| Engine handoff (Unreal/Unity/Godot) | send/receive | Headless connector import of scene layout, cameras, and shot ranges with `director:id` metadata; canonical-space transform return; Unity additionally bakes Director animation onto Timeline, builds Avatars from skinned GLB, and applies PBR material fallback and lights | Requires the Director-authored connector installed in the user's engine project (`nativeReady`); Unreal and Godot do not claim animation, skeletons, or materials; live link is not claimed for any engine |
