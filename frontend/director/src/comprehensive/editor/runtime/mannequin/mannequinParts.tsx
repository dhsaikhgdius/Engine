...
interface ArticulationRingProps {
  color: string;
  name: string;
  position: [number, number, number];
  radius: number;
...
/** Shared material for all humanoid body parts with a matte, slightly metallic finish. */
...
function DetailMaterial({ color = "#070A0F" }: { color?: string }) {
  return <meshStandardMaterial color={color} metalness={0.02} roughness={0.72} />;
}

/** A visible low-poly collar used to make every articulated joint readable in the Stage viewport. */
export function ArticulationRing({
  color,
  name,
  position,
  radius,
  ...
  tube,
}: ArticulationRingProps) {
  return (
    <mesh castShadow receiveShadow name={name} position={position} rotation={rotation}>
      <torusGeometry args={[radius, tube ?? Math.max(radius * 0.19, 0.012), 10, 20]} />
      <meshStandardMaterial color={color} metalness={0.035} roughness={0.52} />
    </mesh>
  );
}

/** A capsule-shaped limb segment (upper arm, forearm, thigh, calf). */
...
/** A spherical joint ball (shoulder, elbow, hip, knee, etc.) with the shared humanoid material. */
export function Joint({ color, name = "humanoid-joint", position, radius, scale = [1, 1, 1] }: JointProps) {
  return (
    <mesh castShadow receiveShadow name={name} position={position} scale={scale}>
...
/** A stylized hand with a spherical palm, thumb capsule, and fingers block. */
export function Hand({ color, position, radius, scale, side }: HandProps) {
  const sideSign = side === "left" ? -1 : 1;

  return (
    <group position={position} scale={scale}>
      <mesh castShadow receiveShadow name={side === "left" ? "humanoid-left-hand" : "humanoid-right-hand"}>
        <sphereGeometry args={[radius, 18, 18]} />
        <HumanoidMaterial color={color} />
      </mesh>
      <mesh
...
/** A capsule-shaped foot with a spherical toe cap, oriented horizontally. */
  ...
  return (
    <group position={position}>
      <mesh
        castShadow
        receiveShadow
        name={side === "left" ? "humanoid-left-foot" : "humanoid-right-foot"}
        rotation={[Math.PI / 2, 0, 0]}
        scale={scale}
      >
...
/** The full torso assembly: chest, abdomen, and pelvis capsules with decorative seam rings. */
export function Torso({
  ...
  return (
    <>
      <mesh castShadow receiveShadow name="humanoid-chest" position={chestPosition} scale={chestScale}>
      ...
      >
        ...
        <DetailMaterial color={accentColor} />
      </mesh>
      <mesh castShadow receiveShadow name="humanoid-abdomen" position={abdomenPosition} scale={abdomenScale}>
      ...
      <mesh castShadow receiveShadow name="humanoid-pelvis" position={pelvisPosition} scale={pelvisScale}>
...
/** The head assembly: neck cylinder, head sphere, and optional facial features (eyes, nose, mouth, muzzle). */
export function Head({
  ...
  return (
    <>
      <mesh castShadow receiveShadow name="humanoid-neck" position={neckPosition}>
        <cylinderGeometry args={[neckRadius * 0.9, neckRadius, neckHeight, 18]} />
        <HumanoidMaterial color={color} />
      </mesh>
      <group position={position} rotation={rotation}>
        <mesh castShadow receiveShadow name="humanoid-head" scale={headScale}>
        ...
        {!featureless ? (
          <>
            <mesh
              castShadow
              receiveShadow
              name="humanoid-face-muzzle"
              position={[0, -headRadius * 0.08, faceOffsetZ]}
              scale={[0.7, 0.52, 0.25]}
            >
            ...
            <mesh
              ...
              receiveShadow
              name="humanoid-nose"
              position={[0, -headRadius * 0.04, faceZ + headRadius * 0.05]}
              scale={noseScale}
            >
            ...
            <mesh
              ...
              receiveShadow
              name="humanoid-mouth"
              position={[0, -headRadius * 0.24, faceZ + headRadius * 0.025]}
              scale={mouthScale}
            >
