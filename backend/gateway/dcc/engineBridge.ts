...
export function createDirectorDccEngineBridge(options: CreateDirectorDccEngineBridgeOptions): DirectorDccEngineBridge {
  ...
  async function send(
    ...
    // the connector keys into its native animation container (Unreal
    // LevelSequence tracks, Godot AnimationPlayer/AnimationLibrary). A bake
    // failure downgrades to a static import with a warning rather than
