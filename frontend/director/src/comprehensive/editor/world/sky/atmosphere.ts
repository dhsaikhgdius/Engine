 ...
 * clearly (snow < overcast < rain < storm at any matched intensity).
 */
 ...
 *
 * The cover term uses the preset-floored effective cover so an overcast sky
 * is hazy even when the authored cover slider sits low; an active weather
 * transition blends the per-preset haze so evolving skies never pop.
 */
export function getAtmosphereMieScale(weather: DirectorWorldWeather, transition?: AtmosphereWeatherTransition): number {
  const cover = evaluateSkyWeatherMood(weather, transition).effectiveCloudCover;
  const extra =
