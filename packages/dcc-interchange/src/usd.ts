...
import type { DirectorObject, DirectorProject, DirectorTransform } from "@director/project-schema";
import {
...
/** MIME type for USDZ archives. */
export const DIRECTOR_USDZ_MIME_TYPE = "model/vnd.usdz+zip";

/** Canonical path for the root USDA layer inside a USDZ archive. */
...
/** Options for importing a Director project from USD. */
...
/**
 * Export a Director project to an ASCII USDA layer.
 * The full project manifest is embedded as base64-encoded custom layer data,
 * while stable IDs and editable transforms remain visible as ordinary prims.
 *
 * @param project - The Director project to export.
 * @param options - Export options (embedProject controls manifest embedding).
 * @returns A USDA 1.0 ASCII string.
 */
...
/**
 * Import a Director project from a USDA ASCII layer.
 * Parses the embedded Director manifest (if present) and Director prim metadata
 * to reconstruct the project, falling back to the provided base project.
 *
 * @param source - The USDA ASCII string to import.
 * @param options - Optional base project for fallback.
 * @returns The reconstructed project and any warnings.
 * @throws If the layer exceeds the size limit, isn't valid USDA 1.0, or uses an incompatible coordinate system.
 */
 ...
 *
 * @param entries - The stored entries to pack into the aligned ZIP.
 * @returns A Uint8Array containing the complete USDZ archive.
 */
...
/**
 * Export a Director project to a USDZ archive (aligned ZIP with USDA + manifest).
 * The archive contains two entries: the root USDA layer and the Director manifest JSON.
 *
 * @param project - The Director project to export.
 * @param options - Export options forwarded to the USDA emitter.
 * @returns A Uint8Array containing the USDZ archive bytes.
 */
...
/**
 * Import a Director project from a USDZ archive.
 * Extracts the USDA root layer and optional Director manifest, validates CRC32,
 * and delegates to the USDA importer.
 *
 * @param source - The USDZ archive as Uint8Array, ArrayBuffer, or Blob.
 * @param options - Optional base project for fallback.
 * @returns The reconstructed project and any warnings.
 * @throws If the archive exceeds limits, fails CRC check, or contains no USDA layer.
 */
