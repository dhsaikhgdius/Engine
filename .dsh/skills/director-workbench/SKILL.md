...
- Overlapping untargeted HTTP/MCP calls now queue on the session reader/writer instead of failing with `session_busy`. Bound calls that carry a `target_token` queue on the exact browser target. Every agent surface (DeepSeek Harness plugin, MCP, CLI) reaches the Gateway through `POST /api/tools/:name`. Read-only `observe` / `query_objects` / `catalog` / `inspect` / `audit` / `diff` calls may run together; mutations still wait for those reads to finish.
