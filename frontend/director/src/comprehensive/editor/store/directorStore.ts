...
export const useDirectorStore = create<DirectorStore>((set, get) => {
  ...
  const resolveCharacterTargets = (target: ObjectMutationTarget) =>
    get().project.objects.filter(
      (item) => item.kind === "character" && ("id" in target ? item.id === target.id : item.crowdId === target.crowdId),
    );
  ...
  return {
    ...
    removeWorldEffects: (effectIds) => {
      ...
      const receipt = dispatchDirectorAuthoringActions([{ action: "remove_world_effects", effect_ids: existingIds }], {
    ...
    removeWorldRoads: (roadIds) => {
      ...
      if (!existingIds.length) return 0;
      const receipt = dispatchDirectorAuthoringActions([{ action: "remove_world_roads", road_ids: existingIds }], {
