...
- `export_exchange_package` works for every provider: a canonical metre/Y-up, stable-ID layout package (scene hierarchy, transforms, cameras). Capability maturity (`native` / `exchange` / `planned`, each with a `layer`) is the honest fidelity claim — trust the per-provider capability table: Unity's connector bakes animation onto Timeline, builds Avatars, and applies PBR material fallback natively, while animation, skeletons, and materials remain exchange-format or planned for Unreal and Godot. Do not promise lossless transfer for any engine.
...
- There is no live engine viewport. Unity ships a preview-only `live_link` (outbound-only long-poll from the Unity Editor with a per-session bearer token and sequence numbers; never authoritative, no remote-execute); `live_link` is still planned for Blender, Unreal, and Godot. The supported baseline for every host is the headless snapshot/patch round trip above.
