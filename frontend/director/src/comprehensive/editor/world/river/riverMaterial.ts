...
export function writeRiverFrameUniforms(
  ...
  uniforms.uRainAgitation.value = computeWaterRainAgitation(weather, climate);  uniforms.uPhase.value = computeWaterDetailPhase(context.seed, body.id);
  uniforms.uTroughLift.value = computeWaterTroughLift(
    riverSurfaceMinY(body),
    body.waveAmplitude * RIVER_WAVE_TROUGH_FACTOR,
    occluderHeight,
  );
  computeWaterSkyReflectionInto(uniforms.uSkyHorizon.value, uniforms.uSkyZenith.value, hours, weather, climate);  computeWaterSunDirectionInto(uniforms.uSunDirection.value, hours);
