/**
 * One film-role tool policy shared by every Director surface.
 *
 * The gateway (MCP, raw HTTP, CLI, hosted harness adapters) and the browser
 * UI consume this same allow-table, so a role that cannot author through
 * `POST /api/tools` also cannot author through a UI control. The module is
 * isomorphic: it imports only protocol-level data and never touches Node or
 * browser runtimes.
 */

import { isFilmRoleId, type FilmRoleId } from "./filmRoles";
import { blenderNativeReadOperationNames } from "./blenderLiveProtocol";
import { asRecord as record } from "./primitives";
