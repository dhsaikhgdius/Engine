...
- Playability evidence is a playtest trace scored into structured issues, each carrying a corrective call — never a compile log, a trailer, or a screenshot. Without a live Stage player session, pass a recorded `trace` for a host-free evaluation. `audit.ready` and a `playable` evaluation are both structural: a game is not done without a playtest, and visual acceptance still requires a Stage capture.
