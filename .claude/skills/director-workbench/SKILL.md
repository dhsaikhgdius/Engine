...
- `export_exchange_package` works for every provider: a canonical metre/Y-up, stable-ID layout package (scene hierarchy, transforms, cameras). Capability maturity (`native` / `exchange` / `planned`, each with a `layer`) is the honest fidelity claim — trust the per-provider capability table: the Unreal connector claims animation/skeletons/materials `native` only for its tested subset (Gateway-baked Sequencer transform/camera tracks, skinned-GLB skeletal import in bind pose, PBR material instances; Control Rig poses, motion clips, and textures warn-and-omit), and Unity's connector bakes animation onto Timeline, builds Avatars, and applies PBR material fallback natively, while animation, skeletons, and materials remain exchange-format or planned for Godot. Do not promise lossless transfer for any engine.
