...
/** The id of a pose preset from the shared pose protocol. */
export type PosePresetId = keyof typeof poseProtocol.presetIds;
/** All valid pose preset ids as a string array. */
...
/** All valid character pose control keys as a string array. */
...
/**
 * Type guard that checks whether a string is a valid character pose control key.
 *
 * @param control - The string to validate.
 * @returns True when the string matches a known control key.
 */
...
/** Min/max bounds for a single pose control value. */
...
/**
 * Clamps a pose control value to its valid range for the given body type.
 *
 * @param control - The pose control key.
 * @param value - The value to clamp.
 * @param bodyType - Optional body type for proportional scaling.
...
/** Complete pose control limits: base categories plus per-control lookup table. */
