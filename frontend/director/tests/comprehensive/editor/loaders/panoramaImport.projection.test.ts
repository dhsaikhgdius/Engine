import * as panoramaImport from "../../../../src/comprehensive/editor/loaders/panoramaImport";
...
it("blends the left and right panorama edges so the sphere seam closes cleanly", () => {
  ...
  const pixels = new Uint8ClampedArray([
    0, 0, 0, 255, 20, 20, 20, 255, 40, 40, 40, 255, 160, 160, 160, 255, 180, 180, 180, 255, 240, 240, 240, 255,
  ]);
...
it("relocates the sphere seam to a low-contrast cut inside the image before wrapping", () => {
  const seamRelocator = (panoramaImport as Record<string, unknown>).relocatePanoramaSeamPixels as
    ((pixels: Uint8ClampedArray, width: number, height: number) => Uint8ClampedArray) | undefined;
  ...
  const height = 2;
  const pixels = new Uint8ClampedArray([
    0, 0, 0, 255, 40, 0, 0, 255, 180, 0, 0, 255, 182, 0, 0, 255, 220, 0, 0, 255, 250, 0, 0, 255, 0, 0, 0, 255, 40, 0, 0,
    255, 180, 0, 0, 255, 182, 0, 0, 255, 220, 0, 0, 255, 250, 0, 0, 255,
  ]);
