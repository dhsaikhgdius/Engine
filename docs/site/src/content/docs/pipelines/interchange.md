...
| Engine handoff (Unreal/Unity/Godot) | send/receive  | Headless connector import of scene layout, cameras, and shot ranges with `director:id` metadata; canonical-space transform return. Unreal additionally keys Gateway-baked transform/camera animation into Sequencer (rational rates, SMPTE start timecode), imports skinned GLBs as bind-pose skeletal meshes, applies Director PBR parameters as material instances, offers a preview-only live-link camera channel, and can return an optional hash-pinned clean-frame render receipt | Requires the Director-authored connector installed in the user's engine project (`nativeReady`); for Unity and Godot, animation, skeletons, materials, and live link are not claimed; the Unreal live link is viewport preview only and never mutates the project; Unreal Control-Rig poses, motion clips, and textures warn-and-omit with structured omission records |
