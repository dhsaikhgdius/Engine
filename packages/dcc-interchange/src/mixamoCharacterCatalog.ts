import mixamoCharacterCatalogJson from "./mixamoCharacterCatalog.json";
...
/** URL path for the local Mixamo character catalog. */
export const MIXAMO_CHARACTER_CATALOG_URL = "/mixamo-characters/catalog.json";

/** Default Mixamo character asset ID used as a fallback when bindings are broken. */
...
/**
 * Look up a Mixamo character catalog item by its asset ID.
 *
...
/**
 * Look up a Mixamo character catalog item by its model URL.
 *
 * @param modelUrl - The model URL to look up.
 * @returns The catalog item, or null if not found.
 */
...
/**
 * Get the default Mixamo character catalog item.
 *
 * @returns The default catalog item.
 * @throws If the default item is missing from the catalog.
 */
...
/**
 * Create a Director asset reference from a Mixamo character catalog item.
 * Deep-clones the character metadata to avoid shared mutable state.
 *
 * @param item - The catalog item to convert.
 * @returns A Director asset reference.
...
/**
 * Get the default Mixamo character as a Director asset reference.
 *
 * @returns A Director asset reference for the default character.
 */
...
/**
 * Validate character asset bindings across the entire project.
 * Used at import, persistence, and Agent boundaries to catch structurally
 * broken bindings. Field drift against the packaged catalog is handled by
 * migration rewrites, so only structurally broken bindings surface here.
 *
...
/** Result of repairing character asset bindings, including the modified project and repair descriptions. */
...
/**
 * Repair structurally broken character bindings by rebinding them to the
 * default packaged character instead of rejecting the load.
 *
 * @param project - The Director project to repair.
 * @returns The repaired project and a list of repair descriptions.
 */
 ...
 * Caches the result and deduplicates concurrent requests. When no fetch
 * function is provided, falls back to the statically bundled catalog.
 *
 * @param request - Optional fetch function for loading the catalog from the network.
 * @returns The parsed and validated model library items.
 */
