...
| Category                             | Count             |
| ------------------------------------ | ----------------- |
| shared Stage project mutators        | 86                |
| ui-only Stage project mutators       | 1                 |
| Coverage (shared / project mutators) | **86 / 87 ≈ 99%** |

Everything still ui-only is deliberately local with a documented divergence
reason in the store (snapshot cameras).
...
| `createObjectList` / `addObjectsToObjectList` / `removeObjectsFromObjectList` / `updateObjectListLabel` | `directorStore.ts` | `create_object_list` / `add_objects_to_object_list` / `remove_objects_from_object_lists` / `rename_object_list`      | shared (the compiled create allocates the same sequential `object_list_N` id and non-crowd membership the legacy writer computes; blank labels, unknown lists, and empty live membership keep the legacy no-op; `observe.objects` and `query_objects` expose `object_list_id` / `object_list_label` / `object_list_detached`, and `query_objects` accepts `object_list_id` as a standalone selector)                                                                                                                 |
...
| `addPresetCharacter` / `addCrowdCharacters` | `directorStore.ts` | `add_object` with explicit `body_type` and the rotating palette `color`; crowd adds carry `crowd_id` / `crowd_label`, one action per member in one atomic batch | shared (the compiled adds allocate the same sequential `char_preset_N` ids, names, grid positions, and palette colors the legacy writer computes; the engine injects the default Mixamo asset from the packaged catalog when the project lacks it, and the palette no longer hands out the migration-rewritten legacy blue) |
