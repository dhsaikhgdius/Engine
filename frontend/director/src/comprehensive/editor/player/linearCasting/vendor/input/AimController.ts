// @ts-nocheck
// Ported from LinearAbiltyCastingThreeJS (MIT) — https://github.com/achrefelouafi/LinearAbiltyCastingThreeJS
...
export class AimController extends EventEmitter {
  constructor(camera) {
    ...
    /** World-space floor height. Director scenes are not always at y = 0. */
    this.groundHeight = 0;
  }

  setGroundHeight(height) {
    this.groundHeight = Number.isFinite(height) ? height : 0;
    GROUND_PLANE.constant = -this.groundHeight;
  }
  ...
  setOrigin(position) {
    this.origin.set(position.x, this.groundHeight, position.z);
  }
