...
- There is no authoritative live engine viewport. Unreal `live_link` is `native` as a preview-only loopback camera channel (live frames are never applied as project mutations). Unity `live_link` is `native` as a preview-only outbound-only long-poll from the Unity Editor (per-session bearer token, monotonic sequence numbers; never authoritative, no remote-execute). Blender's native kernel additionally exposes a bounded preview-only live-link delta feed (`blender_native {"op":"live_link"}` with a `{sceneEpoch, seq}` cursor, replay-protected, resync on gap or epoch change). Godot `live_link` stays planned. The supported durable channel is always the headless snapshot/patch round trip above.
