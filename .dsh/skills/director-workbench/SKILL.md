...
- Playability evidence is a playtest trace scored into structured issues, each carrying a corrective call — never a compile log, a trailer, or a screenshot. Gateway playtest prefers a connected Stage tab; otherwise it runs the host-free kinematic tape (or accepts an explicit `trace`). `audit.ready` and a `playable` evaluation are both structural: a game is not done without a playtest, and visual acceptance still requires a Stage capture.
