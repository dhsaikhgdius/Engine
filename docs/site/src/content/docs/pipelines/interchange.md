...
| Blender round trip | export/return | Validated scene/camera/light handoff, clay preview, stable-ID return of meshes, transforms, camera optics, `director_id` lights, and portable pose controls | Return is limited to hashed packages below the DCC job root; Blender-only objects are not auto-imported, and direct armature bone edits are warned, not reconciled |
| Engine handoff (Unreal/Unity/Godot) | send/receive | Headless connector import of scene layout, cameras, and shot ranges with `director:id` metadata; canonical-space transform return. Unreal additionally keys Gateway-baked transform/camera animation into Sequencer (rational rates, SMPTE start timecode), imports skinned GLBs as bind-pose skeletal meshes, applies Director PBR parameters as material instances, offers a preview-only live-link camera channel, and can return an optional hash-pinned clean-frame render receipt; Unity additionally bakes Director animation onto Timeline, builds Avatars from skinned GLB, and applies PBR material fallback and lights; Godot 4 additionally imports Gateway-baked `AnimationPlayer` animation on a rational timebase, skinned GLB skeletons in bind pose, `StandardMaterial3D` materials with hashed external textures, and Omni/Spot/Directional lights | Requires the Director-authored connector installed in the user's engine project (`nativeReady`); the Unreal live link is viewport preview only and never mutates the project, and live link is not claimed for Unity or Godot; Unreal Control-Rig poses, motion clips, and textures warn-and-omit with structured omission records; Godot rig pose channels and ambient/rect lights warn-and-omit |
