...
- There is no live engine viewport for Unreal/Unity/Godot: their `live_link` remains planned and the supported baseline is the headless snapshot/patch round trip above. Blender's native kernel additionally exposes a bounded preview-only live-link delta feed (`blender_native {"op":"live_link"}` with a `{sceneEpoch, seq}` cursor, replay-protected, resync on gap or epoch change). It is never authoritative: committed Director state only changes through revision-guarded command batches or the reviewed return import.
