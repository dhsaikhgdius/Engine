...
| Engine handoff (Unreal/Unity/Godot) | send/receive | Headless connector import of scene layout, cameras, and shot ranges with `director:id` metadata; canonical-space transform return; Unity additionally bakes Director animation and semantic pose channels onto Timeline, builds Avatars from skinned GLB, applies PBR material fallback and lights, and offers an outbound-only preview live link | Requires the Director-authored connector installed in the user's engine project (`nativeReady`); Unreal and Godot do not claim animation, skeletons, materials, or live link; Unity's live link is preview-only and never scene authority |
