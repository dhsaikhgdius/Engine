...
`assets/` is the WorldEngine asset directory, split into **source asset catalogs (library)** and **runtime-generated artifacts (generated)**. Assets are distributed via Hugging Face datasets, and `manifest.lock.json` (a version-pinned manifest) describes the full supply chain: source repository, commit SHA, SHA256 checksum, and license reference. Run `npm run assets:install` to pull the required assets from Hugging Face during development.
