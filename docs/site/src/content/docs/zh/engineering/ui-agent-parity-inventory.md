...
- Canvas 节点/边/布局/置顶创作（`canvas.node.*`、含 `canvas.node.bring_to_front`、`canvas.edge.*`、
  `canvas.dag.layout`）、素材导入入册（`gallery.media.update`）以及撤销/重做按钮（`workspace.undo` /
  `workspace.redo`）。
