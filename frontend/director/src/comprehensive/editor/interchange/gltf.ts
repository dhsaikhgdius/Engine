...
/** MIME type for glTF JSON (`.gltf`). */
export const DIRECTOR_GLTF_MIME_TYPE = "model/gltf+json";
/** MIME type for glTF Binary (`.glb`). */
export const DIRECTOR_GLB_MIME_TYPE = "model/gltf-binary";
/** Maximum byte size for a glTF/GLB interchange payload. */
export const DIRECTOR_GLTF_MAX_BYTES = 128 * 1024 * 1024;

/** Root-level metadata embedded in the glTF asset extras. */
...
/** Per-node metadata identifying Director entities within the glTF scene graph. */
export interface DirectorGltfEntityMetadata {
  adapter: typeof DIRECTOR_GLTF_ADAPTER;
  contract: typeof DIRECTOR_INTERCHANGE_CONTRACT;
  /** Stable interchange ID for round-trip fidelity. */
  stableId: string;
  /** Whether this node represents a Stage object or a camera. */
  entityType: "object" | "camera";
  /** The Director object kind (character, prop, scene, etc.). */
  kind?: DirectorObject["kind"];
  /** The asset reference ID that backs this object. */
  assetRefId?: string;
  /** The parent object ID for hierarchical grouping. */
  parentObjectId?: string;
}

/** A glTF JSON document with its associated binary resources. */
export type DirectorGltfDocument = JSONDocument;

/** Options for importing a glTF interchange document. */
export interface ImportDirectorGltfOptions {
  /** Optional base project to merge into; if absent, a new project is created. */
...
/**
 * Builds a glTF document from a Director project.
 *
 * Stage objects become Xform nodes with Director metadata in extras.
 * Cameras become glTF Camera nodes with orientation derived from the
 * look-at target (rather than the rig model's persisted Euler rotation).
 *
 * @param project - The Director project to export.
 * @returns A glTF document with Director metadata.
 */
...
/**
 * Serializes a Director project to a glTF JSON document.
 *
 * @param project - The Director project to export.
 * @returns A glTF JSON document ready for serialization.
 */
...
/**
 * Serializes a Director project to a pretty-printed glTF JSON string.
 *
 * @param project - The Director project to export.
 * @param pretty - Whether to pretty-print (default true).
 * @returns A glTF JSON string.
 */
...
/**
 * Serializes a Director project to a glTF Binary (GLB) byte array.
 *
 * @param project - The Director project to export.
...
/**
 * Imports a Director project from glTF JSON (string, parsed JSON, or JSONDocument).
 *
 * @param source - The glTF JSON source.
...
/**
 * Imports a Director project from GLB (glTF Binary) bytes.
 *
 * @param source - The GLB bytes.
 * @param options - Import options.
 * @returns The imported project and any warnings.
 */
