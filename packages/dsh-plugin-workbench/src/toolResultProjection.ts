...
/**
 * Removes inline base64 media payloads (`data` / `dataBase64` next to a
 * `mimeType`) anywhere in a tool result, so encoded images never reach the
 * model as text. The media itself travels through the surface's native
 * attachment or image-block channel instead.
 */
export function stripEncodedMediaPayloads(value: unknown): unknown {
...
/**
 * Convenience wrapper used by the model-facing surfaces (DSH plugin, MCP):
 * projects an envelope only when it needs projection, without spilling.
 *
 * @param tool - The Director tool that produced the envelope.
 * @param envelope - The model-facing envelope.
 * @returns The original envelope, or a slimmed projection of it.
 */
export function projectOversizedDirectorAgentToolEnvelope(
  tool: string,
  envelope: Record<string, unknown>,
): Record<string, unknown> {
  const decision = directorAgentToolResultNeedsProjection(envelope, { tool, input: undefined });
