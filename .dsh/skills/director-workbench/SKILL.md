...
- There is no authoritative live engine viewport: Unreal `live_link` is `native` as a preview-only loopback camera channel (live frames are never applied as project mutations); `live_link` stays planned for Blender, Unity, and Godot. The supported durable channel is always the headless snapshot/patch round trip above.
