...
function getSeamBlendWidth(width: number) {
  return Math.max(
    PANORAMA_SEAM_MIN_WIDTH,
    Math.min(PANORAMA_SEAM_MAX_WIDTH, Math.round(width * PANORAMA_SEAM_BLEND_RATIO)),
  );
}

function getPoleBlendHeight(height: number) {
  return Math.max(
    PANORAMA_POLE_MIN_HEIGHT,
    Math.min(PANORAMA_POLE_MAX_HEIGHT, Math.round(height * PANORAMA_POLE_BLEND_RATIO)),
  );
}
...
/**
 * Blends the left and right edges of an equirectangular panorama to eliminate
 * visible seam artifacts at the 0°/360° boundary.
 *
 * The blend is a linear cross-fade over the specified seam width.
 *
 * @param pixels - The RGBA pixel data.
 * @param width - Image width in pixels.
 * @param height - Image height in pixels.
 * @param seamWidth - The number of pixel columns to blend on each edge.
 * @returns A new `Uint8ClampedArray` with the seam blended.
 */
export function blendPanoramaSeamPixels(pixels: Uint8ClampedArray, width: number, height: number, seamWidth: number) {
...
/**
 * Softens the top and bottom poles of an equirectangular panorama.
 *
 * Pole regions are blended toward the average color of the reference row,
 * reducing visible compression artifacts at the zenith and nadir.
 *
 * @param pixels - The RGBA pixel data.
 * @param width - Image width in pixels.
 * @param height - Image height in pixels.
 * @param poleBlendHeight - The number of pixel rows to blend from each pole.
 * @returns A new `Uint8ClampedArray` with poles softened.
 */
...
/**
 * Finds the column with the lowest energy transition for seam relocation.
 *
 * Scans each column (excluding the first) and computes a transition score
 * based on the absolute color difference between adjacent columns. The column
 * with the lowest score is the best place to cut and relocate the seam.
 *
 * @param pixels - The RGBA pixel data.
 * @param width - Image width in pixels.
 * @param height - Image height in pixels.
 * @returns The column index (1-based) with the lowest energy transition.
 */
...
/**
 * Relocates the panorama seam to the specified column by cyclically shifting
 * the pixel rows.
 *
 * When the seam is at column 0, the image is unchanged.
 *
 * @param pixels - The RGBA pixel data.
 * @param width - Image width in pixels.
 * @param height - Image height in pixels.
 * @param seamColumn - The column to become the new left edge; defaults to the lowest-energy column.
 * @returns A new `Uint8ClampedArray` with the seam relocated.
 */
...
function drawImageContain(context: CanvasRenderingContext2D, source: CanvasImageSource, placement: ContainPlacement) {
...
/**
 * Reads a panorama image file and adapts it for use as a skybox.
 *
 * Already-equirectangular images (2:1 ratio) are used as-is. Non-panoramic
 * images are projected onto a 2:1 canvas with cover placement, seam relocation,
 * and pole softening to approximate an equirectangular projection.
 *
 * @param file - The image file selected by the user.
 * @returns The panorama asset with id, fileName, projection mode, and URL.
 * @throws If the file extension is not supported (JPG, PNG, WEBP).
 */
