import {
  ...
  resolveMannequinModelUrl,
} from "../../../../../src/comprehensive/editor/runtime/ue4Mannequin/ue4MannequinRig";

it("points to the built-in rigged GLB asset", () => {
  expect(UE4_MANNEQUIN_MODEL_URL).toBe(
    resolveDirectorAssetUrl(import.meta.env.BASE_URL, "models/storyai-open-mannequin.glb"),
  );
});

it("resolves the built-in rigged GLB below the embedded director desk base path", () => {
  expect(resolveDirectorAssetUrl("/director-desk/", "models/storyai-open-mannequin.glb")).toBe(
    "/director-desk/models/storyai-open-mannequin.glb",
  );
  expect(resolveDirectorAssetUrl("./", "models/storyai-open-mannequin.glb")).toBe(
    "./models/storyai-open-mannequin.glb",
  );
  expect(resolveMannequinModelUrl("./", "/extensions/ComfyUI-3D-Director/director_standalone.html")).toBe(
    DIRECTOR_MANNEQUIN_API_URL,
  );
  expect(resolveMannequinModelUrl("/preview/", "/preview/index.html")).toBe(
    "/preview/models/storyai-open-mannequin.glb",
  );
});
