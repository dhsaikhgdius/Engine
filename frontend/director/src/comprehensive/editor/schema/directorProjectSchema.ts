...
import { strictMode } from "../../../../../../packages/protocol/src/strictProtocolVariant";
import { directorCameraAspectRatioSchema } from "../../../../../../packages/protocol/src/directorCameraProtocol";
import { referenceSceneReconstructionPlanSchema } from "../../../../../../packages/protocol/src/referenceSceneReconstructionProtocol";
import { directorProceduralRecipeSchema } from "../../../../../../packages/protocol/src/directorProceduralProtocol";
import {
...
/** A 3-element tuple of finite numbers representing a 3D vector. */
...
/** The source type of an asset (model, image, audio, etc.). */
export const directorAssetSourceTypeSchema = z.enum(DIRECTOR_ASSET_SOURCE_TYPES);
/** The kind of scene object (character, prop, camera, light, etc.). */
export const directorObjectKindSchema = z.enum(DIRECTOR_OBJECT_KINDS);
/** Whether an animation entity is an object or a camera. */
export const directorAnimationEntityTypeSchema = z.enum(["object", "camera"]);

/**
 * Creates a bounded number schema for a camera optics parameter.
 *
 * @param key - The camera optics parameter name (e.g. "apertureFStop", "iso").
 * @returns A zod schema clamped to the parameter's valid range.
 */
...
/** PBR texture slot-to-asset-id bindings, keyed by texture slot name. */
...
/** PBR material parameters: base color, metalness, roughness, opacity, and textures. */
...
/** A scene light with type, color, intensity, position, and shadow settings. */
...
/** A clipping plane that hides geometry on one side of a world-space plane. */
...
/** A named object layer with visibility and lock state. */
...
/** A position anchor that can optionally bind to a scene object. */
...
/** A named object layer for visibility/locking grouping. */
...
/** Path action parameters for a camera that follows a trajectory. */
export type DirectorCameraPathAction = Extract<ParsedDirectorCameraAction, { mode: "path" }>["path"];
/** Follow action parameters for a camera that tracks a target. */
...
/** Re-exported vehicle profile types from the shared protocol package. */
export type {
  DirectorVehicleProfile,
  DirectorVehicleKind,
} from "../../../../../../packages/protocol/src/vehicleProtocol";
/** The full Director project document, with cameras typed permissively. */
export type DirectorProject = Omit<z.infer<typeof directorProjectSchema>, "cameras"> & {
  cameras: DirectorCameraShot[];
};
/** The scene-level settings block of a Director project. */
export type SceneSettings = DirectorProject["scene"];

/**
 * Parses and validates an unknown value as a Director project.
 *
 * Throws a ZodError on failure. Callers that need user-facing error messages
 * should use {@link safeParseDirectorProject} instead.
 *
 * @param value - The untrusted JSON payload to parse.
 * @returns A fully validated Director project.
 */
...
/**
 * Safe parse of an unknown value as a Director project.
 *
 * Returns a tagged union so callers can branch on success without catching
 * exceptions. The error message is localized to Chinese.
 *
 * @param value - The untrusted JSON payload to parse.
 * @returns Either a valid project or a user-facing error string.
 */
...
/**
 * Safe parse of an unknown value against the structural-only schema.
 *
 * Structural validation checks field shapes and duplicate-id corruption.
 * Dangling references are left for repair, so a structurally valid document
 * can still open in the editor.
 *
 * @param value - The untrusted JSON payload to parse.
 * @returns Either a valid project or a user-facing error string.
 */
...
/** The result of repairing a structurally valid project's dangling references. */
export interface DirectorProjectRepairResult {
  /** The repaired project with all references resolved or cleaned up. */
  project: DirectorProject;
  /** Human-readable (Chinese) descriptions of each repair applied. */
  repairs: string[];
}
 ...
 *
 * Repairs include: annotation/measurement anchors to deleted objects, world
 * effect anchors, wildlife asset references, material texture bindings, and
 * production take/sequence references. Each repair produces a human-readable
 * description of what was changed.
 *
 * @param project - A structurally validated Director project.
 * @returns The repaired project with a list of applied repairs.
 */
